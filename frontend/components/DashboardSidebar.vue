  ...
  <div>
    ...
    >
      <div class="flex flex-col h-full">
        ...
        <!-- Region Selector -->
        <div v-if="availableRegions.length > 1" class="px-3 py-2 border-b border-gray-200/50" data-region-switcher>
          <div class="relative">
            <button
            ...
            >
              <div
                ...
                class="absolute left-0 right-0 mt-1 bg-white/95 backdrop-blur-md rounded-lg border border-gray-200/50 shadow-lg py-0.5 z-10 max-h-64 overflow-y-auto"
              >
                <!-- All Regions option -->
                <button
                  @click="selectRegion(null)"
                  class="w-full flex items-center gap-2 px-2.5 py-1.5 hover:bg-gray-50 transition-colors"
                  :class="!selectedRegion && 'bg-gray-100'"
                >
                  <span class="text-sm">🌐</span>
                  <span class="flex-1 text-xs font-medium text-gray-700 text-left">All Regions</span>
                  <svg
                    v-if="!selectedRegion"
                    class="w-3.5 h-3.5 text-brand flex-shrink-0"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    stroke-width="2"
                ...
                <button
                  ...
                  class="w-full flex items-center gap-2 px-2.5 py-1.5 hover:bg-gray-50 transition-colors"
                  :class="selectedRegion === region.code && 'bg-gray-100'"
                >
                  <span class="text-sm">{{ region.flag }}</span>
                  <span class="flex-1 text-xs font-medium text-gray-700 text-left">{{ region.name }}</span>
                  <svg
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto sidebar-scroll">
          ...
          <NuxtLink
            ...
            @click="mobileMenuOpen = false"
          >
            <component :is="HomeIcon" class="w-5 h-5 flex-shrink-0 transition-colors" :class="route.path === '/dashboard' ? 'text-brand' : 'text-gray-400 group-hover:text-gray-600'" />
            <span>Dashboard</span>
          </NuxtLink>
          ...
          <div class="pt-3">
            <button
              @click="expandedSections.analytics = !expandedSections.analytics"
              class="flex items-center justify-between w-full px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider hover:text-gray-600 transition-colors"
            >
              <span>Analytics</span>
              <svg class="w-3.5 h-3.5 transition-transform" :class="expandedSections.analytics && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <div v-show="expandedSections.analytics" class="mt-1 space-y-0.5">
              <NuxtLink
                v-for="item in analyticsItems"
                :key="item.path"
                :to="item.path"
                class="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 group whitespace-nowrap"
                :class="isActive(item.path) ? 'bg-brand/10 text-brand shadow-sm' : 'text-gray-600 hover:bg-gray-100/80 hover:text-gray-900'"
                @click="mobileMenuOpen = false"
              >
                <component :is="item.icon" class="w-4 h-4 flex-shrink-0 transition-colors" :class="isActive(item.path) ? 'text-brand' : 'text-gray-400 group-hover:text-gray-600'" />
                <span>{{ item.name }}</span>
              </NuxtLink>
            </div>
          </div>
          ...
          <div class="pt-2">
            <button
              @click="expandedSections.improvement = !expandedSections.improvement"
              class="flex items-center justify-between w-full px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider hover:text-gray-600 transition-colors"
            >
              <span>Improvement</span>
              <svg class="w-3.5 h-3.5 transition-transform" :class="expandedSections.improvement && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <div v-show="expandedSections.improvement" class="mt-1 space-y-0.5">
              <template v-for="item in improvementItems" :key="item.path">
                <!-- Disabled item (Coming Soon) -->
                <div
                  v-if="item.disabled"
                  class="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium text-gray-400 cursor-not-allowed whitespace-nowrap"
                >
                  <component :is="item.icon" class="w-4 h-4 flex-shrink-0 text-gray-300" />
                  <span>{{ item.name }}</span>
                  <span v-if="item.badge" class="ml-auto px-1.5 py-0.5 text-[10px] font-medium rounded-full" :class="item.badgeClass">
                    {{ item.badge }}
                  </span>
                </div>
                <!-- Active item -->
          ...
          <div class="pt-2">
            <button
              @click="expandedSections.management = !expandedSections.management"
              class="flex items-center justify-between w-full px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider hover:text-gray-600 transition-colors"
            >
              <span>Management</span>
              <svg class="w-3.5 h-3.5 transition-transform" :class="expandedSections.management && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <div v-show="expandedSections.management" class="mt-1 space-y-0.5">
              <NuxtLink
          ...
          <div class="pt-2">
            <button
              @click="expandedSections.reports = !expandedSections.reports"
              class="flex items-center justify-between w-full px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider hover:text-gray-600 transition-colors"
            >
              <span>Reports</span>
              <svg class="w-3.5 h-3.5 transition-transform" :class="expandedSections.reports && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <div v-show="expandedSections.reports" class="mt-1 space-y-0.5">
              <template v-for="item in reportsItems" :key="item.path">
                <!-- Disabled item (Coming Soon) -->
                <div
                  v-if="item.disabled"
                  class="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium text-gray-400 cursor-not-allowed whitespace-nowrap"
          ...
          <div class="pt-4 mt-2 border-t border-gray-100 space-y-0.5">
            <NuxtLink
...
onMounted(async () => {
  ...
  // Load regions after products are loaded so we have the active product ID
  await loadAvailableRegions(activeProductId.value)
...
// Refresh regions when active product changes
watch(activeProductId, async (newProductId) => {
  if (newProductId) {
    await refreshRegions(newProductId)
  }
})
...
const GlobeIcon = () => h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
...
const analyticsItems = [
  ...
  { name: 'Regions', path: '/dashboard/regions', icon: GlobeIcon },
  { name: 'Sources', path: '/dashboard/sources', icon: LinkIcon },
  { name: 'Competitors', path: '/dashboard/competitors', icon: UsersIcon },
]

const improvementItems = [
  { name: 'Recommendations', path: '/dashboard/recommendations', icon: LightbulbIcon, badge: 'Beta', badgeClass: 'bg-blue-100 text-blue-700' },
  { name: 'Gaps', path: '/dashboard/gaps', icon: GapsIcon, badge: 'Coming Soon', badgeClass: 'bg-gray-100 text-gray-500', disabled: true },
  { name: 'Freshness', path: '/dashboard/freshness', icon: ClockIcon, badge: 'Coming Soon', badgeClass: 'bg-gray-100 text-gray-500', disabled: true },
]
...
const reportsItems = [
  ...
  { name: 'ROI Calculator', path: '/dashboard/roi', icon: CurrencyIcon, badge: 'Coming Soon', badgeClass: 'bg-gray-100 text-gray-500', disabled: true },
]

// Bottom navigation items (always visible, not in a collapsible section)
const bottomItems = [
  { name: 'Run Scans', path: '/dashboard/extension', icon: DesktopIcon },
...
const autoExpandSection = () => {
  ...
  if (analyticsItems.some(item => path.startsWith(item.path))) {
    expandedSections.value.analytics = true
  } else if (improvementItems.some(item => path.startsWith(item.path))) {
    expandedSections.value.improvement = true
  } else if (managementItems.some(item => path.startsWith(item.path))) {
    expandedSections.value.management = true
  } else if (reportsItems.some(item => path.startsWith(item.path))) {
...
<style scoped>
/* Custom compact scrollbar for sidebar navigation */
.sidebar-scroll {
