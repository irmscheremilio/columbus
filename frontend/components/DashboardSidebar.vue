    ...
    >
      <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          ...
          <div class="pt-2">
            ...
            <div v-show="expandedSections.improvement" class="mt-1 space-y-0.5">
              <template v-for="item in improvementItems" :key="item.path">
                ...
                <div
                  v-if="item.disabled"
                  class="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium text-gray-400 cursor-not-allowed"
                >
          ...
          <div class="pt-2">
            <button
              @click="expandedSections.tools = !expandedSections.tools"
              class="flex items-center justify-between w-full px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider hover:text-gray-600 transition-colors"
            >
              <span>Tools</span>
              <svg class="w-3.5 h-3.5 transition-transform" :class="expandedSections.tools && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <div v-show="expandedSections.tools" class="mt-1 space-y-0.5">
              <NuxtLink
              ...
              >
                <component :is="item.icon" class="w-4 h-4 flex-shrink-0 transition-colors" :class="isActive(item.path) ? 'text-brand' : 'text-gray-400 group-hover:text-gray-600'" />
                <span>{{ item.name }}</span>
                <span v-if="item.badge" class="ml-auto px-2 py-0.5 text-xs font-medium rounded-full" :class="item.badgeClass || 'bg-brand/10 text-brand'">
                  {{ item.badge }}
                </span>
              </NuxtLink>
            </div>
...
const autoExpandSection = () => {
  ...
  } else if (toolsItems.some(item => path.startsWith(item.path))) {
