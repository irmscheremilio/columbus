          ...
          <table class="w-full">
            <thead>
              <tr class="text-[11px] text-gray-500 uppercase tracking-wider border-b border-gray-100/80 bg-gray-50/30">
                <th class="text-left px-4 py-3 font-medium w-[140px]">Platform</th>
                <th class="text-left px-4 py-3 font-medium">Prompt</th>
                <th class="text-center px-4 py-3 font-medium w-[80px]">Mentioned</th>
                <th class="text-center px-4 py-3 font-medium w-[80px] hidden sm:table-cell" title="AI used sources in response">Sources</th>
                <th class="text-center px-4 py-3 font-medium w-[90px] hidden sm:table-cell" title="Brand website was cited">Brand Cited</th>
                <th class="text-center px-4 py-3 font-medium w-[70px] hidden md:table-cell">Position</th>
                <th class="text-center px-4 py-3 font-medium w-[90px] hidden lg:table-cell">Sentiment</th>
                <th class="text-center px-4 py-3 font-medium w-[60px] hidden lg:table-cell">Limit</th>
                <th class="text-right px-4 py-3 font-medium w-[100px]">Date</th>
              </tr>
            </thead>
              ...
              >
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2.5">
                    <div class="w-7 h-7 rounded-lg bg-gray-50 group-hover:bg-gray-100 flex items-center justify-center flex-shrink-0 transition-colors">
                      <img
                        ...
                        @error="($event.target as HTMLImageElement).style.display = 'none'"
                      />
                      <span v-else class="text-[10px] font-bold text-gray-500">{{ formatModelName(result.ai_model).charAt(0) }}</span>
                    </div>
                    <span class="text-xs font-medium text-gray-700 hidden sm:inline">{{ formatModelName(result.ai_model) }}</span>
                  </div>
                </td>
                <td class="px-4 py-3">
                  <div class="text-gray-900 truncate max-w-xs text-sm" :title="result.prompt">{{ result.prompt }}</div>
                </td>
                <td class="px-4 py-3 text-center">
                  <div class="flex items-center justify-center">
                    <span
                      class="inline-flex w-6 h-6 rounded-full items-center justify-center"
                      :class="result.brand_mentioned ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-400'"
                    >
                      <svg v-if="result.brand_mentioned" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                ...
                <td class="px-4 py-3 text-center hidden sm:table-cell">
                  <div class="flex items-center justify-center">
                    ...
                    >
                      <svg v-if="result.has_sources" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                      </svg>
                      <span v-else class="text-xs">−</span>
                    </span>
                ...
                <td class="px-4 py-3 text-center hidden sm:table-cell">
                  <div class="flex items-center justify-center">
                    ...
                    >
                      <svg v-if="result.citation_present" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                      </svg>
                      <span v-else class="text-xs">−</span>
                    </span>
                ...
                <td class="px-4 py-3 text-center hidden md:table-cell">
                  <span v-if="result.position" class="inline-flex items-center justify-center min-w-[24px] h-6 px-1.5 rounded-md bg-blue-50 text-blue-700 text-xs font-semibold">
                    #{{ result.position }}
                  </span>
                ...
                <td class="px-4 py-3 text-center hidden lg:table-cell">
                  ...
                  >
                    <span v-if="result.sentiment === 'positive'" class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                    <span v-else-if="result.sentiment === 'negative'" class="w-1.5 h-1.5 rounded-full bg-red-500"></span>
                ...
                <td class="px-4 py-3 text-center hidden lg:table-cell">
                  <span
                    ...
                    class="inline-flex w-6 h-6 rounded-full items-center justify-center bg-amber-100 text-amber-600"
                ...
                <td class="px-4 py-3 text-right">
                  <span class="text-xs text-gray-500">{{ formatDate(result.tested_at) }}</span>
                </td>
              </tr>
            </tbody>
          </table>
