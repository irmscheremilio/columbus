  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100">
    <div class="p-4 lg:p-6 space-y-5">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 class="text-lg sm:text-xl font-semibold text-gray-900 tracking-tight">Regional Analysis</h1>
          <p class="text-xs sm:text-sm text-gray-500">Compare AI visibility across different regions</p>
        </div>
        <DateRangeSelector />
      </div>
      ...
      <div v-if="loading" class="flex items-center justify-center py-12 sm:py-20">
        <div class="w-6 h-6 sm:w-8 sm:h-8 animate-spin rounded-full border-2 border-brand border-t-transparent"></div>
      </div>

      <!-- No Data State -->
      <div v-else-if="regionStats.length === 0" class="text-center py-12 sm:py-20 px-4 bg-white/80 backdrop-blur-sm rounded-xl border border-white/50 shadow-sm">
        ...
        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-2">No regional data yet</h3>
      ...
      <template v-else>
          ...
          <div class="px-3 sm:px-5 py-3 sm:py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-l-4 border-l-brand">
            <div class="flex items-center gap-3 sm:gap-4">
              <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0">
                <span class="text-xl sm:text-2xl">{{ getCountryFlag(bestRegion?.region || 'local') }}</span>
              </div>
              <div>
                <div class="text-[10px] sm:text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-0.5">Best Performing Region</div>
                <div class="flex items-baseline gap-1.5 sm:gap-2">
                  <span class="text-2xl sm:text-3xl font-bold text-gray-900">{{ bestRegion?.mentionRate || 0 }}</span>
                  <span class="text-base sm:text-lg font-medium text-gray-400">%</span>
                </div>
                <div class="text-[10px] sm:text-xs text-gray-500 mt-0.5">{{ getCountryName(bestRegion?.region || 'local') }}</div>
              </div>
            </div>
            <!-- Mobile: horizontal scroll stats -->
            <div class="flex items-center gap-2 sm:gap-3 overflow-x-auto -mx-3 px-3 sm:mx-0 sm:px-0 pb-1 sm:pb-0">
              <div class="text-center px-2 sm:px-3 py-1 sm:py-1.5 bg-gray-50 rounded-lg flex-shrink-0">
                <div class="text-sm sm:text-base font-bold text-gray-900">{{ regionStats.length }}</div>
                <div class="text-[9px] sm:text-[10px] text-gray-500 whitespace-nowrap">Regions</div>
              </div>
              <div class="text-center px-2 sm:px-3 py-1 sm:py-1.5 bg-gray-50 rounded-lg flex-shrink-0">
              ...
              <div class="text-center px-2 sm:px-3 py-1 sm:py-1.5 bg-gray-50 rounded-lg flex-shrink-0">
                <div class="text-sm sm:text-base font-bold text-gray-900">{{ avgMentionRate }}%</div>
                <div class="text-[9px] sm:text-[10px] text-gray-500 whitespace-nowrap">Avg Rate</div>
              </div>
              <div class="text-center px-2 sm:px-3 py-1 sm:py-1.5 bg-amber-50 rounded-lg flex-shrink-0">
                <div class="text-sm sm:text-base font-bold text-amber-600">{{ variancePercent }}%</div>
                <div class="text-[9px] sm:text-[10px] text-amber-600 whitespace-nowrap">Variance</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div class="bg-white/90 backdrop-blur-sm rounded-xl shadow-sm border border-white/50 overflow-hidden hover:shadow-md transition-shadow duration-200">
            <div class="px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-100/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                <div class="w-1 h-3 sm:h-4 rounded-full bg-brand"></div>
                <div>
                  <h2 class="text-xs sm:text-sm font-semibold text-gray-900">Mention Rate by Region</h2>
                  <p class="text-[9px] sm:text-[10px] text-gray-500 mt-0.5">Regional performance comparison</p>
                </div>
              </div>
              <div class="flex items-center gap-1 bg-gray-100/80 rounded-lg p-0.5 self-start sm:self-auto">
                <button
                  v-for="metric in metrics"
                  :key="metric.value"
                  @click="selectedMetric = metric.value"
                  class="px-2 sm:px-2.5 py-1 text-[10px] sm:text-xs font-medium rounded-md transition-all duration-200"
          ...
          <div class="bg-white/90 backdrop-blur-sm rounded-xl shadow-sm border border-white/50 overflow-hidden hover:shadow-md transition-shadow duration-200">
            <div class="px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-100/80 flex items-center gap-2">
              ...
              <div>
                <h2 class="text-xs sm:text-sm font-semibold text-gray-900">Region Rankings</h2>
                <p class="text-[9px] sm:text-[10px] text-gray-500 mt-0.5">Sorted by mention rate</p>
              </div>
            </div>
            <div class="p-3 sm:p-4">
              <div class="space-y-2 max-h-48 sm:max-h-64 overflow-y-auto pr-1">
                <div
                  v-for="(stat, idx) in regionStats"
                  :key="stat.region"
                  class="flex items-center gap-2 sm:gap-3"
                >
                  <span class="text-[10px] sm:text-xs text-gray-400 w-3 sm:w-4 font-medium flex-shrink-0">{{ idx + 1 }}</span>
                  <span class="text-base sm:text-xl flex-shrink-0">{{ getCountryFlag(stat.region) }}</span>
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between mb-0.5 sm:mb-1">
                      <div class="flex items-center gap-1 sm:gap-1.5">
                        <span class="font-medium text-xs sm:text-sm text-gray-900 truncate">{{ getCountryName(stat.region) }}</span>
                        <span
                          v-if="idx === 0"
                          class="px-1 sm:px-1.5 py-0.5 text-[8px] sm:text-[10px] font-medium bg-emerald-100 text-emerald-700 rounded-full flex-shrink-0"
                        >
                          Best
                        </span>
                      </div>
                      <span class="text-xs sm:text-sm font-semibold text-gray-900 flex-shrink-0">{{ stat.mentionRate }}%</span>
                    </div>
                    ...
                    <div class="text-[9px] sm:text-[10px] text-gray-400 mt-0.5">{{ stat.total }} tests · {{ stat.citationRate }}% cited</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="bg-white/90 backdrop-blur-sm rounded-xl shadow-sm border border-white/50 overflow-hidden hover:shadow-md transition-shadow duration-200">
          <div class="px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-100/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <div class="w-1 h-3 sm:h-4 rounded-full bg-violet-500"></div>
              <div>
                <h2 class="text-xs sm:text-sm font-semibold text-gray-900">Platform Performance by Region</h2>
                <p class="text-[9px] sm:text-[10px] text-gray-500 mt-0.5">Mention rates for each AI platform across regions</p>
              </div>
            </div>
            <div class="flex items-center gap-1.5 sm:gap-2 text-[9px] sm:text-xs">
              <span class="flex items-center gap-0.5 sm:gap-1"><span class="w-2 h-2 sm:w-3 sm:h-3 rounded bg-emerald-100"></span> ≥50%</span>
              <span class="flex items-center gap-0.5 sm:gap-1"><span class="w-2 h-2 sm:w-3 sm:h-3 rounded bg-amber-100"></span> 25-49%</span>
              <span class="flex items-center gap-0.5 sm:gap-1"><span class="w-2 h-2 sm:w-3 sm:h-3 rounded bg-red-100"></span> &lt;25%</span>
            </div>
          </div>
            ...
            <table class="w-full">
              <thead>
                <tr class="text-[10px] sm:text-xs text-gray-500 uppercase tracking-wide border-b border-gray-100/80">
                  <th class="text-left px-3 sm:px-4 py-2 sm:py-3 font-medium bg-gray-50/50">Platform</th>
                  <th
                    v-for="stat in regionStats"
                    :key="stat.region"
                    class="text-center px-2 sm:px-4 py-2 sm:py-3 font-medium"
                  >
                    <div class="flex flex-col items-center gap-0.5">
                      <span class="text-sm sm:text-base">{{ getCountryFlag(stat.region) }}</span>
                      <span class="text-[8px] sm:text-[10px] text-gray-400 normal-case hidden sm:block">{{ getCountryName(stat.region) }}</span>
                    </div>
                  </th>
                  <th class="text-center px-2 sm:px-4 py-2 sm:py-3 font-medium bg-gray-50/50">
                    <span class="hidden sm:inline">Average</span>
                    <span class="sm:hidden">Avg</span>
                  </th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100/50">
                <tr v-for="platform in matrixPlatforms" :key="platform" class="hover:bg-gray-50/50 transition-colors">
                  <td class="px-3 sm:px-4 py-2 sm:py-3 bg-gray-50/30">
                    <div class="flex items-center gap-1.5 sm:gap-2">
                      ...
                      />
                      <span class="font-medium text-[10px] sm:text-sm text-gray-900 truncate max-w-[60px] sm:max-w-none">{{ formatModelName(platform) }}</span>
                    </div>
                  </td>
                  <td
                    v-for="stat in regionStats"
                    :key="`${platform}-${stat.region}`"
                    class="px-1.5 sm:px-4 py-2 sm:py-3 text-center"
                  >
                    <span
                      class="inline-flex px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-lg text-[10px] sm:text-xs font-semibold min-w-[2rem] sm:min-w-[3rem] justify-center"
                  ...
                  <td class="px-2 sm:px-4 py-2 sm:py-3 text-center bg-gray-50/30">
                    <span class="text-[10px] sm:text-sm font-semibold text-gray-700">
              ...
              <tfoot class="border-t border-gray-200">
                <tr class="bg-gray-50/50">
                  <td class="px-3 sm:px-4 py-2 sm:py-3 font-medium text-[10px] sm:text-sm text-gray-700">
                    <span class="hidden sm:inline">Region Average</span>
                    <span class="sm:hidden">Avg</span>
                  </td>
                  <td
                    v-for="stat in regionStats"
                    :key="`avg-${stat.region}`"
                    class="px-2 sm:px-4 py-2 sm:py-3 text-center"
                  >
                    <span class="text-[10px] sm:text-sm font-semibold text-gray-700">{{ stat.mentionRate }}%</span>
                  </td>
                  <td class="px-2 sm:px-4 py-2 sm:py-3 text-center">
                    <span class="text-[10px] sm:text-sm font-bold text-brand">{{ avgMentionRate }}%</span>
                  </td>
                </tr>
              </tfoot>
            </table>
          ...
          >
            <div class="px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-100/80 flex items-center justify-between">
              <div class="flex items-center gap-2 sm:gap-3">
                <span class="text-xl sm:text-2xl">{{ getCountryFlag(stat.region) }}</span>
                <div>
                  <h3 class="font-semibold text-gray-900 text-xs sm:text-sm">{{ getCountryName(stat.region) }}</h3>
                  <p class="text-[9px] sm:text-[10px] text-gray-500">{{ stat.total }} tests</p>
                </div>
              </div>
              <div class="text-right">
                <div class="text-base sm:text-lg font-bold" :class="idx === 0 ? 'text-brand' : 'text-gray-900'">{{ stat.mentionRate }}%</div>
                <div class="text-[9px] sm:text-[10px] text-gray-400">mention rate</div>
              </div>
            </div>
            <div class="p-3 sm:p-4">
                  ...
                  <div class="flex items-center justify-between mb-0.5 sm:mb-1">
                    <div class="flex items-center gap-1.5 sm:gap-2">
                      <img
                        ...
                        :alt="ps.platform"
                        class="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded"
                        @error="($event.target as HTMLImageElement).style.display = 'none'"
                      />
                      <span class="text-[10px] sm:text-xs text-gray-600 truncate max-w-[80px] sm:max-w-none">{{ formatModelName(ps.platform) }}</span>
                    </div>
                    <span class="text-[10px] sm:text-xs font-semibold text-gray-900">{{ ps.mentionRate }}%</span>
                  </div>
                  <div class="h-1 sm:h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      class="h-full rounded-full transition-all duration-500"
              ...
              <div class="mt-3 sm:mt-4 pt-2.5 sm:pt-3 border-t border-gray-100 flex items-center justify-between text-[10px] sm:text-xs">
