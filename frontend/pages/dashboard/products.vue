  ...
  <div>
    <main class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <div class="px-4 py-6 sm:px-0">
        ...
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-gray-500">Manage your products and track their AI visibility</p>
            </div>
          </div>
          <button
            @click="showAddModal = true"
            :disabled="!canAddProduct"
            class="flex items-center gap-2 px-4 py-2 bg-brand text-white rounded-lg font-medium hover:bg-brand/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <!-- Product Slots Info -->
        <div v-if="productSlots" class="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              <div class="text-sm text-gray-600">
                <span class="font-medium text-gray-900">{{ productSlots.used }}</span> of
                <span class="font-medium text-gray-900">{{ productSlots.total }}</span> product slots used
              </div>
        ...
        <div v-else-if="products.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div
            ...
            :class="{ 'ring-2 ring-brand': activeProductId === product.id }"
            @click="selectProduct(product.id)"
          >
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-lg bg-brand/10 flex items-center justify-center text-brand font-semibold">
                  {{ product.name.charAt(0).toUpperCase() }}
                </div>
                <div>
                  <h3 class="font-semibold text-gray-900">{{ product.name }}</h3>
                  <a :href="`https://${product.domain}`" target="_blank" class="text-sm text-gray-500 hover:text-brand">
                    {{ product.domain }}
                  </a>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <span
                  ...
                  class="px-2 py-0.5 bg-brand/10 text-brand text-xs font-medium rounded"
                >
                  Active
                </span>
                <button
            ...
            <div class="mb-4">
              <div class="flex items-center justify-between mb-1">
                ...
                <span class="text-sm font-medium" :class="getScoreColor(product.aeo_score)">
                  {{ product.aeo_score ?? '--' }}
                </span>
              </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-100 flex gap-2">
              <button
                ...
                class="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-brand/10 text-brand rounded-lg text-sm font-medium hover:bg-brand/20 transition-colors disabled:opacity-50"
              >
        ...
        <div v-else class="text-center py-20 bg-white rounded-xl border border-gray-200">
          ...
          <button
            @click="showAddModal = true"
            class="inline-flex items-center gap-2 px-6 py-3 bg-brand text-white rounded-lg font-medium hover:bg-brand/90 transition-colors"
          >
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
            </svg>
      ...
      <div v-if="showAddModal || showEditModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="closeModals">
        <div class="bg-white rounded-xl p-6 w-full max-w-md mx-4">
          ...
          <form @submit.prevent="showEditModal ? handleUpdateProduct() : handleAddProduct()" class="space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Website Domain <span class="text-red-500">*</span></label>
              <input
                v-model="productForm.domain"
                type="text"
                :disabled="showEditModal"
                class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand disabled:bg-gray-100 disabled:cursor-not-allowed"
                placeholder="example.com"
                required
              />
              <p v-if="showEditModal" class="text-xs text-gray-500 mt-1">Domain cannot be changed after creation</p>
            </div>
            <div>
              ...
              <textarea
                ...
                rows="3"
                class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand resize-none"
